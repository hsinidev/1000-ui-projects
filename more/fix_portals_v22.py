...
# Icons that are purely decorative and shouldn't trigger a match on their own
ICONS_TO_IGNORE = {
...
def find_match(text, norm_map, current_screen, exclude_current=True):
    ...
    # Filter out icons unless it's the ONLY thing there
    ...
    for phrase in search_phrases:
        for folder_norm, target in norm_map.items():
            ...
            # Prefix or substring match (more lenient)
...
def smart_link(html_content, screens_map, current_screen, all_screens_content):
    ...
    # 1. Match a tags, buttons, and clickable containers
    # We include nav, header, footer in the search to be more aggressive
    ...
    def unified_callback(match):
        ...
        if a_tag:
            ...
            if inner_match:
                full_prefix, prefix, href, text, suffix = inner_match.groups()
                if href.startswith(("http", "mailto", "tel", "javascript")): return a_tag
                
                # Check if it's inside a header/footer/nav/aside to be more aggressive with fallbacks
                # (This is hard with regex, so we'll just be more aggressive for all a tags)
                
                match_target = find_match(text, norm_map, current_screen, exclude_current=(href == current_screen))
                
                if not match_target:
                    # Robust fallback for ANY link that is '#' or point to current page
    ...
    # 2. Wrap loose icons in headers/nav/footers/aside that aren't clickable
    # We'll search for blocks of header/footer/nav/aside and wrap spans/divs with icons
    blocks_pattern = re.compile(r'(?is)<(header|footer|nav|aside)[^>]*>.*?</\1>')
    
    def block_callback(block_match):
        content = block_match.group(0)
        # Wrap spans/divs that have icons and aren't already links
        # This regex looks for span/div that contains material-symbols-outlined but NO <a> ancestor
        icon_wrapper_pattern = re.compile(r'(?is)<(span|div|button)\s+([^>]*class=["\'][^"\']*material-symbols-outlined[^"\']*["\'][^>]*>)(.*?)</\1>')
        
        def icon_callback(icon_match):
            tag_name, attrs, inner = icon_match.group(1), icon_match.group(2), icon_match.group(3)
            # If already inside an <a> tag (heuristically), don't wrap
            # (In a single pass we can't be sure, but we can check if it's preceded by <a> or followed by </a> closely)
            # But the block_callback is safer.
            
            # Find target based on icon name or surrounding text
            match_target = find_match(inner, norm_map, current_screen) or "index.html"
            
            # Check if this icon was ALREADY wrapped by our previous unified_pattern (which turned it into an <a> tag)
            # Wait, the unified_pattern only touches things with cursor-pointer.
            
            return f'<a href="{match_target}" style="display:inline-block; text-decoration:none; color:inherit;">{icon_match.group(0)}</a>'

        # We only wrap if NOT already inside an <a> tag
        # A simple way to avoid double wrapping is to skip if <a> is found in the match or nearby
        # For now, let's just use the unified_pattern and make it more aggressive.
        return content

    # Instead of block_callback, let's just update unified_pattern to be more inclusive of headers/navs
    # NEW PATTERN: Includes elements inside header/footer/nav/aside even if they don't have cursor-pointer
    # Actually, a better way is to just target ALL span/div/li that contain an icon in these sections.

    html_content = unified_pattern.sub(unified_callback, html_content)

    # 3. Final Sweep for remaining '#' in header/nav
    # Match any <a> with href="#" in a block that looks like a nav
    final_sweep = re.compile(r'(?i)<a\s+[^>]*href=["\']#["\'][^>]*>(.*?)</a>')
    html_content = final_sweep.sub(lambda m: m.group(0).replace('href="#"', 'href="index.html"').replace("href='#'", "href='index.html'"), html_content)

    # 4. Global Navigator (Explore Screens)
    nav_html = """
    ...
    if "</body>" in html_content: 
        # Remove existing stitch-nav if present
...
def process_project(folder_name):
    ...
    for target_filename in screens_map.values():
        ...
        # If it's index.html, also save a copy with its original name for internal links consistency
