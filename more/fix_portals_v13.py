...
ICONS_TO_IGNORE = {
    ...
    "menu", "close", "expand_more", "chevron_right"
}
...
def smart_link(html_content, screens_map, current_screen, all_screens_content):
    ...
    def find_match(text):
        ...
        if any(w in clean_text for w in ["dashboard", "home", "logo", "back", "inicio", "overview", "race control", "main hub"]):
            return "index.html"

        search_phrases = [clean_text]
        if len(filtered_words) > 1: search_phrases.extend(filtered_words)

        # Tier 1: Direct Folder Name Match (Exact or Start)
        ...
        # Tier 4: Content Search (Title/H1 only)
        for phrase in search_phrases:
        ...
        # Tier 5: General Content Search
        for phrase in search_phrases:
            if len(phrase) < 5: continue
            for target, content in all_screens_content.items():
                if target == "index.html": continue
                if phrase in content.lower(): return target
    ...
    def replace_btn(match):
        ...
        if 'onclick' in tag_open.lower(): return match.group(0) # Skip JS buttons
        match_target = find_match(text)
        if match_target:
    ...
    # Aggressive interactive element conversion (div, li, span with cursor-pointer or hover classes)
