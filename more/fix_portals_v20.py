...
ICONS_TO_IGNORE = {
    ...
    "notifications", "settings", "person", "emergency", "hub", "dashboard"
}
...
def smart_link(html_content, screens_map, current_screen, all_screens_content):
    ...
    def find_match(text):
        ...
        # Filter out ignored icons unless it's the only word
        ...
        # 1. Exact or Prefix match in folder names (Highest Priority)
        ...
        # 4. Priority Home Triggers (Dashboard, Home, etc.)
        home_triggers = ["dashboard", "home", "logo", "back", "inicio", "overview", "race control", "main hub", "portal", "operations", "city control"]
        if any(w in clean_text for w in home_triggers):
            return "index.html"

        # 5. Deep content match (Last Resort)
        for phrase in search_phrases:
            if len(phrase) < 4: continue
            for target, content in all_screens_content.items():
                if target == "index.html": continue
                lower_content = content.lower()
                # Check title/h1 specifically
    ...
    def unified_callback(match):
        ...
        if a_tag:
            ...
            if inner_match:
                ...
                # FALLBACK LOGIC: If no specific match, but it's a standard utility icon/text, point to index.html
                if not match_target:
                ...
                if match_target:
                    # Aggressively update placeholders or current-page links
