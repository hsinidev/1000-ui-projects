...
def find_home_screen(screens, inner_folder):
    ...
    # Priority keywords for true landing pages
    ...
    # Second pass: contains but not necessarily a word
    for p in priorities:
        for screen in valid_screens:
...
def smart_link(html_content, screens_map, current_screen):
    ...
    def find_match(text):
        ...
        # 1. Exact match with a folder name (excluding home if we have others)
        ...
        # 2. Specialized synonyms with word boundaries
        synonyms = {
            "locations": ["map", "pickup", "find", "address", "reach", "contact"],
            "fleet": ["cars", "vehicles", "garage", "inventory", "models", "showroom"],
            "booking": ["reserve", "appointment", "concierge", "book", "schedule"],
            "concierge": ["booking", "service", "support", "help", "assistant"],
            "experience": ["360", "virtual", "tour", "explore"],
            "home": ["index", "main", "inicio", "start", "welcome", "logo"],
            "about": ["metadata", "info", "project", "story", "legacy"]
        }
        
        for key, syns in synonyms.items():
            if text == key or any(s == text for s in syns):
                # Try to find a folder matching this key or its syns
                for folder_norm, target in norm_map.items():
                    if key in folder_norm or any(s in folder_norm for s in syns):
                        if target != "index.html" or len(norm_map) == 1:
                            return target
        
        # 3. Fallback: match index.html for home-like terms
    ...
    # Add Screen Navigator (discrete overlay)
    nav_html = """
...
def process_project(folder_name):
    ...
    for d, target_filename in screens_map.items():
        ...
        fixed_html = smart_link(html, screens_map, d)
