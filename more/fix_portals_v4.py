...
def find_home_screen(screens, inner_folder):
    # Only consider screens that actually have code.html
    valid_screens = []
    for s in screens:
...
def process_project(folder_name):
    ...
    # List subdirectories that are screens
    ...
    if not home_screen:
        # Check if index.html is generic
        index_path = os.path.join(project_path, "index.html")
        if os.path.exists(index_path):
            with open(index_path, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read()
                if "Stitch UI Portfolio" in content:
                    print(f"Generic index found for {folder_name} but no valid home screen code. Keeping it.")
        return

    # Delete existing index.html if it's generic
    index_path = os.path.join(project_path, "index.html")
    if os.path.exists(index_path):
        with open(index_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read()
