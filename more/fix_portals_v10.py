...
# Common Material Icon names to ignore during text matching
ICONS_TO_IGNORE = {
...
def smart_link(html_content, screens_map, current_screen, all_screens_content):
    ...
    def find_match(text):
        # 1. Clean the text: remove icons and junk
        ...
        # Check various lengths of the text (e.g. "Mission Control" from "rocket_launch Mission Control")
        # We try to match any "meaningful" part of the text
        search_phrases = [clean_text]
        if len(filtered_words) > 1:
            # Try individual words if it's a long phrase
            search_phrases.extend(filtered_words)

        for phrase in search_phrases:
            # 1. Exact match with folder
            ...
            # 2. Synonyms
            synonyms = {
                "locations": ["map", "pickup", "find", "address", "reach", "contact", "navigation", "destinations"],
            ...
            # 3. Content-Aware Matching
            for target, content in all_screens_content.items():
                if target == "index.html" and current_screen == "index.html": continue
                if phrase in content.lower():
                    if f"<title>{phrase}" in content.lower() or f"<h1>{phrase}" in content.lower() or f"<h2>{phrase}" in content.lower():
                        return target
                    return target
        
        if any(w in clean_text for w in ["home", "logo", "back", "inicio"]):
            return "index.html"
