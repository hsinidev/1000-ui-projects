...
def inject_seo(html_path, project_name):
    try:
        ...
        # Remove any personal-footer div
        html = re.sub(r'<div class="personal-footer".*?</div>', '', html, flags=re.DOTALL)
        # Remove legacy copyright strings that might have leaked
        html = re.sub(r'<p style="margin-top: 40px; color: rgba\(255,255,255,0\.15\);.*?© 2024 HSINI\.DEV.*?</p>', '', html, flags=re.DOTALL)
        # Remove JSON-LD Schema (previous versions)
        html = re.sub(r'<!-- JSON-LD Schema -->.*?({.*?})?.*?</script>', '', html, flags=re.DOTALL)
        # Remove legacy SEO meta tags
        ...
        # Remove any double/triple closing tags that might have accumulated from faulty replacements
        # (This is a bit risky but we see </div></div> leakage)
        # Better: remove the footer block completely if it exists.
    ...
    """
        ...
        # 4. FOOTER BRANDING
        footer_branding = f"""
    <div class="personal-footer" style="margin: 80px auto 40px; padding: 40px 20px; border-top: 1px solid rgba(255,255,255,0.05); text-align: center; font-family: sans-serif; background: rgba(0,0,0,0.3); width: 100%; max-width: 1200px; border-radius: 24px; backdrop-filter: blur(10px); position: relative; z-index: 10;">
    ...
    """
        ...
        if '</body>' in html:
            # We insert BEFORE </body> but also before the stitch-nav if it exists
            if '<div id="stitch-nav"' in html:
                html = html.replace('<div id="stitch-nav"', f"{footer_branding}\n<div id=\"stitch-nav\"")
            else:
...
def process_all_portals():
    ...
    for folder in project_folders:
        ...
        print(f"Branding & SEO Optimized (v28 - Cleanup & Icons): {folder}")
