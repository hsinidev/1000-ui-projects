...
def smart_link(html_content, screens_map, current_screen, all_screens_content):
    ...
    def find_match(text, current_href=None):
        ...
        synonyms = {
            ...
            "experience": ["360", "virtual", "tour", "explore", "cabin", "lounge", "view"],
    ...
    # Replace <a> tags (even if not #)
    a_pattern = re.compile(r'(<a\s+[^>]*href=["\']([^"\']*)["\'][^>]*>)(.*?)(</a>)', re.IGNORECASE | re.DOTALL)
    def replace_a(match):
        prefix, href, text, suffix = match.groups()
        # If it's an external link or complex path, ignore
        if href.startswith(("http", "mailto", "tel", "javascript")): return match.group(0)
        
        match_target = find_match(text, href)
        if match_target:
            # Only replace if the target is different or if the current one is likely a placeholder
            if href == "#" or href == "" or href == current_screen:
                return prefix.replace(f'href="{href}"', f'href="{match_target}"') + text + suffix
            # Logic: If current href is "driver_performance_profile.html" but text is "PIT STOPS", it's a placeholder.
            # We check if match_target matches the text BETTER than the current href.
            if ".html" in href:
                current_name = href.replace(".html", "").replace("_", " ")
                match_name = match_target.replace(".html", "").replace("_", " ")
                # If current name has NO relation to the text, but match_name DOES, then swap.
                text_clean = re.sub(r'<[^>]+>', ' ', text).lower()
                if not any(w in current_name for w in text_clean.split()) and match_target != href:
                     return prefix.replace(f'href="{href}"', f'href="{match_target}"') + text + suffix
